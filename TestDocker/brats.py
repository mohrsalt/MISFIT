...
def get_brats_dataset(data_paths, csv_path=None, phase="train"):
    ...
    for data_path in data_paths:
        for subject in os.listdir(data_path):
            ...
            matches_t1n = glob.glob(os.path.join(sub_path, f"{subject}-t1n.nii.gz"))
            t1n = matches_t1n[0] if matches_t1n else None

            matches_t1c = glob.glob(os.path.join(sub_path, f"{subject}-t1c.nii.gz"))
            t1c = matches_t1c[0] if matches_t1c else None

            matches_t2w = glob.glob(os.path.join(sub_path, f"{subject}-t2w.nii.gz"))
            t2w = matches_t2w[0] if matches_t2w else None

            matches_t2f = glob.glob(os.path.join(sub_path, f"{subject}-t2f.nii.gz"))
            t2f = matches_t2f[0] if matches_t2f else None
